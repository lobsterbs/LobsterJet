...
export const COMPAT: Record<string, CompatEntry> = {
  ...
  "browser.scripting": { supported: "partial", reason: "executeScript/insertCSS with declared files, dispatched to the engine UI which injects into the same-origin proxied frame and acks back; func injection cannot cross the SW boundary and is unsupported" },
  ...
  "webNavigation.*": { supported: "partial", reason: "onCommitted/onCompleted derived from top-level tab URL changes; frameId always 0, completion fires with commit (no per-load signal)" },
  "contextMenus.*": { supported: "partial", reason: "registration registry; the engine UI surfaces items and reports clicks through the SW" },
  "notifications.*": { supported: "no", reason: "requires the LobsterBrowse notification surface" },
  "downloads.*": { supported: "partial", reason: "download dispatches to the engine UI with ack; search over the registry record; no pause/resume/cancel yet" },
